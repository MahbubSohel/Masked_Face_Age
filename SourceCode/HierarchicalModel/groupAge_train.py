...
def main():
    ...
    df=pd.read_csv('/home/cviplab/MaskAge/SourceCode/groupage/3G_v2/3Gv2_group_Dist.csv')
    wghts= df['Weights']
...
def train(train_loader, model, criterion1, optimizer, epoch):
    ...
    for i, (inp, target, groupTarget) in enumerate(train_loader):
        ...
        if use_gpu:
            ...
            target_var = torch.autograd.Variable(groupTarget.long().cuda())

        else:
            input_var = torch.autograd.Variable(input1.float())
            target_var = torch.autograd.Variable(groupTarget.long())
        output_var = model(input_var)
        loss = criterion1(output_var, target_var)

        acc1, acc5 = accuracy(output_var.data, target_var.data, topk=(1, 2))
    ...
    f = open('Train_3Gv2_WRN_LW_grAge.txt', 'a')
...
def validate(val_loader, model, criterion, epoch):
    ...
    f = open('Val_3Gv2_WRN_LW_grAge.txt', 'a')
    f.write('Epoch: {0}\t''Loss: {loss.avg:.4f}\t''acc@1 {top1.avg:.3f}\t''acc@5 {top5.avg:.3f}\n'.format(epoch,
                                                                                                          loss=losses,
                                                                                                          top1=top1,
                                                                                                          top5=top5))
...
def save_checkpoint(state, is_best, epoch, filename='ckp_3Gv2_WRN_LW_grAge.pth.tar'):
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, '3Gv2_WRN_LW_grAge.pth.tar')
