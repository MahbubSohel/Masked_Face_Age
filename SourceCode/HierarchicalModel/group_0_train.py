...
def main():
    ...
    df=pd.read_csv('/home/cviplab/MaskAge/SourceCode/group_0/3G_v2/3Gv2_0_dist.csv')
    wghts= df['Weights']
    ...
    train_data = AgePredictionData(csv_file='/home/cviplab/MaskAge/file_paths/3G_v2/group_0_train.csv',
    ...
    valid_data = AgePredictionData(csv_file='/home/cviplab/MaskAge/file_paths/3G_v2/group_0_val.csv',
                                   root_dir='/home/cviplab/MaskAge/DataSet/CropMask/',
                                   transform=transforms.Compose([
                                       Rescale(256),
                                       RandomCrop(224),
...
def train(train_loader, model, criterion1, optimizer, epoch):
    ...
    f = open('Train_3Gv2_0_WRN_LW.txt', 'a')
...
def validate(val_loader, model, criterion, epoch):
    ...
    f = open('Val_3Gv2_0_WRN_LW.txt', 'a')
    f.write('Epoch: {0}\t''Loss: {loss.avg:.4f}\t''acc@1 {top1.avg:.3f}\t''acc@5 {top5.avg:.3f}\n'.format(epoch,
                                                                                                          loss=losses,
                                                                                                          top1=top1,
                                                                                                          top5=top5))
...
def save_checkpoint(state, is_best, epoch, filename='ckp_3Gv2_0_WRN_LW.pth.tar'):
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, '3Gv2_0_WRN_LW.pth.tar')


def adjust_learning_rate(optimizer, epoch):
    """Sets the learning rate to the initial LR decayed by 10 every 30 epochs"""
    lr = L_Rate * (0.1 ** (epoch // 10))
